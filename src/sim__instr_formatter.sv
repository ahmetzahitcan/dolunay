...
(
    ...
    input wire logic [RLEN-1:0] imm_i,
    input wire logic [RLEN-1:Z_PC] pc_i,
    output sim__disasm_t disasm_o
);

    `ifdef SYNTHESIS
        // FIXME: This probably does nothing (synthesis may ignore initial block entirely)
        initial $fatal(1, "sim__instr_formatter cannot be synthesized");
    `endif
    ...
    always_comb begin
        ...
        s = $sformatf("%0d:", pc_i);
        foreach (s[i]) begin
            disasm_o[disasm_idx*8 +: 8] = unsigned'(s[i]);
            disasm_idx--;
        end
        ...
        foreach (format_i[i]) begin
            if (special) begin
                ...
                foreach (s[j]) begin
                    disasm_o[disasm_idx*8 +: 8] = unsigned'(s[j]);
                    disasm_idx--;
                end
                ...
                if (format_i[i] == "$") begin
                    ...
                    disasm_o[disasm_idx*8 +: 8] = unsigned'(format_i[i]);
                    disasm_idx--;
                end
            end
        end
