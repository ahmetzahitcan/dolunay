...
(
    ...
    always_comb begin
        op1_w = 'x;
        unique case (instr_i.alu_op1_sel)
    ...
    always_comb begin
        op2_w = 'x;
        unique case (instr_i.alu_op2_sel)
    ...
    always_comb begin
        addy_op1_w = 'x;
        unique case (instr_i.alu_addy_funct)
    ...
    assign addy_result_w = addy_op1_w + addy_op2_w + {{(RLEN-1){1'b0}}, addy_cin_w};

    logic [RLEN-1:0] result_w;
    always_comb begin
        result_w = 'x;
        unique case (instr_i.alu_funct)
            ALU_FUNCT_ADDY: result_w = addy_result_w;
            ALU_FUNCT_SLL: result_w = op1_w << op2_w[4:0];
            ALU_FUNCT_SLT: result_w = {{(RLEN-1){1'b0}}, $signed(op1_w) < $signed(op2_w)};
            ALU_FUNCT_SLTU: result_w = {{(RLEN-1){1'b0}}, $unsigned(op1_w) < $unsigned(op2_w)};
            ALU_FUNCT_XOR: result_w = op1_w ^ op2_w;
            ALU_FUNCT_SRL: result_w = op1_w >> op2_w[4:0];
            ALU_FUNCT_SRA: result_w = unsigned'($signed(op1_w) >>> op2_w[4:0]);
